...
export default <RouterOptions>{
  routes: (_routes) => {
    ...
    if (subdomain.value) {
      const userRoute = _routes.filter((i) => i.path.includes(`/landing/${subdomain.value}`))
      const userRouteMapped = userRoute.map((i) => ({
        ...i,
        path: i.path === `/landing/${subdomain.value}` ? i.path.replace(`/landing/${subdomain.value}`, `/`) : i.path.replace(`/landing/${subdomain.value}/`, `/`),
      }))
